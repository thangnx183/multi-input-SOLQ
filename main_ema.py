...
from util.tool import load_model, ModelEMA
...
def main(gpu, ngpus_per_node, args):
    ...
    if args.input_mode == 'single':
        collate_fn = utils.collate_fn_single
    else:
    ...
    ema = ModelEMA(model) if args.gpu in {-1,0} else None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm, args.input_mode)
        if ema :
            # print('check ',ema)
            ema.update(model)
        lr_scheduler.step()
